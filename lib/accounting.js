...
export async function syncSystemAccountingEntries() {
  ...
  const [transactions, expenses, purchaseOrders] = await Promise.all([
    Transaction.find({ status: { $in: ["completed", "refunded"] } })
      .select("_id createdAt refundedAt refundReason status total tax tenderType tenderPayments location staffName items")
      .lean(),
  ...
  const operations = [
    ...transactions.map((transaction) => postSaleEntry(transaction)),
    ...transactions
  ...
  return {
    salesSynced: transactions.length,
