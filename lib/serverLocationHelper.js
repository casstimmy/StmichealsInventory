  ...
  try {
    ...
    for (const store of stores) {
      ...
      store.locations.forEach((loc, idx) => {
        ...
        // Convert ObjectId to string properly - handle both ObjectId instances and string types
        let locId = null;
        if (loc._id) {
          locId = typeof loc._id === 'object' && loc._id.toString 
            ? loc._id.toString() 
            : String(loc._id);
        }
        ...
        if (locId) {
          // Map by ObjectId string (24-char hex string)
